...
path = raw_input("Please enter the path of the root directory:") #Example : /home/PP/Courses/
...
for x in range(len(duplicates)):
    for y in range(x+1,len(duplicates)):
        if(duplicates[x][2]==duplicates[y][2]):
            ...
            num=int(raw_input("Enter the file number you want to delete (Enter 0 to skip):"))
            if(num==1):
